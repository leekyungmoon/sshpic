...
if ($PSVersionTable.PSVersion.Major -lt 7) {
    throw 'sshpic Windows uninstall requires PowerShell 7. Open PowerShell 7, then run ./scripts/windows/uninstall.ps1.'
}
...
function Resolve-SshpicGitSh {
    ...
    throw 'Git for Windows sh.exe was not found. Install Git for Windows, then rerun ./scripts/windows/uninstall.ps1.'
}
...
try {
    ...
    $previousFacadeState = $env:SSHPIC_UNINSTALL_POWERSHELL_FACADE
    ...
    try {
        ...
        if ($null -eq $previousFacadeState) {
            Remove-Item Env:\SSHPIC_UNINSTALL_POWERSHELL_FACADE -ErrorAction SilentlyContinue
        }
        else {
            $env:SSHPIC_UNINSTALL_POWERSHELL_FACADE = $previousFacadeState
        }
        Pop-Location
    }
