...
6. If the focused iTerm2 session is local Codex rather than SSH, sshpic stores a bounded local image copy under `~/.sshpic/images/clipboard.png` and inserts that local path.
7. If the focused iTerm2 session is neither SSH nor a recognized local coding-agent session, sshpic delegates to native Paste and does not upload or insert a path.
