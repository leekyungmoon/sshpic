...
func TestLuaIntegrationSourceUsesFocusedAsyncNativePasteContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		"is_focused_plink", "info.argv[2] == '-load'", "info.argv[3] == 'sshpic-managed-password-upstream-v1'",
		"info.argv[5] == '-share'", "info.argv[9] == '-noagent'", "info.argv[10] == '-no-trivial-auth'",
		"is_focused_codex", "forward_paste_key", "result.kind == 'local_image'",
		"wezterm.action.SendKey { key = 'v', mods = 'ALT' }",
		"not is_focused_ssh(info) and not is_focused_plink(info) and not is_focused_codex(info)",
		...
		"Focused Plink is not the managed password SSH upstream",
