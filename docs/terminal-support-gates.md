...
- installation starts with the public `./scripts/windows/install.ps1` command in PowerShell 7; before setup is declared ready, that same PowerShell PID must change `Get-Command ssh` from the native application to the managed function while preserving `ssh.exe`;
