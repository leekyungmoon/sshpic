...
- One cross-platform `./install.sh` entry point, run from an already-open Git Bash window on Windows, with optional `winget` provisioning for Go and WezTerm plus `install`, `doctor`, and `restore` coverage for WezTerm.
