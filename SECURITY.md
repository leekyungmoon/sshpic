...
The iTerm2 integration uses Python RPC only when the iTerm2 runtime is ready. Runtime-missing no-Python `Cmd+V` hooks are disabled because real Mac testing showed they can pollute ordinary paste. `sshpic` does not continuously watch clipboard changes; it acts when the installed paste shortcut is invoked.
